...
int main()
{
    ...
    rep(i,n)
    {
        ...
        cin >>x >>y;
        s[i].pb(x.substr(0,3));
        s[i].pb(x.substr(0,2)+y.substr(0,1));
    }
    ...
    rep(i,n)rep(j,i)
    {
        rep(ii,2)rep(jj,2)
        {
            if(s[i][ii] == s[j][jj]) solver.add_clause(i,ii,j,jj);
        }

        if(s[i][0]==s[j][0])
        {
            solver.add_clause(i,true,j,false);
            solver.add_clause(j,true,i,false);
        }
    }
    ...
    {
        ans = "YES";
        vector<bool> literals = solver.get_literals();
        rep(i,n)
        {
            int idx = 1;
            if(literals[i]) idx = 0;
