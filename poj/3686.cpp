...
const int MAX_V = 3000; // TODO:initialize
...
int main()
{
    ...
    while(CASE--)
    {
        ...
        int z[50][50];
        scanf(" %d %d", &n, &m);
        rep(i,n)rep(j,m) scanf(" %d", &z[i][j]);

        // 0 ~ n-1 : toy
        // n  ~ 2n-1 : 1st factory
        // 2n  ~ 3n-1 : 2nd factory
        // mn  ~ (m+1)n-1 : mth factory
        ...
        rep(i,n)rep(j,m)rep(k,n) add_edge(i,(j+1)*n+k,1,z[i][j]*(k+1));

        rep(j,m)rep(k,n) add_edge((j+1)*n+k,T,1,0);

        double ans = min_cost_flow(S,T,n);
        printf("%.6f\n", ans/n);
    }
    return 0;
}
