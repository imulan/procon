...
class Decipherability {
    ...
    string check(string s, int K) {
        int S=s.size();
        string ok="Certain", ng="Uncertain";

        if(S==K) return ok;

        rep(i,S)for(int j=i+1; j<S; ++j)
        {
            if(s[i]==s[j] && j-i<=K) return ng;
        }
...
// CUT begin
ifstream data("Decipherability.sample");
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "Decipherability (250 Points)" << endl << endl;
    }
