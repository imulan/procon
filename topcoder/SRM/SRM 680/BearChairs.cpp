...
class BearChairs {
    ...
    vector<int> findPositions(vector<int> atLeast, int d) {
      ...
      for(i=1;i<atLeast.size();++i){
        ...
        if(atLeast[i]<=v[0]-d) ins=atLeast[i];
        else{
          bool found=false;
          ...
          rep(j,v.size()-1){
            ...
            int right=v[j+1]-d;
            //範囲に収まっていればそこを選ぶ
            if(left<=atLeast[i] && atLeast[i]<=right){
              found=true;
              ins=atLeast[i];
              break;
            }

            if(atLeast[i]<=left && left<=right){
              found=true;
              ins=left;
              break;
            }
          ...
          if(!found) ins=max(atLeast[i],v[v.size()-1]+d);
        }
