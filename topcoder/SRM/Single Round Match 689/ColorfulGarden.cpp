...
class ColorfulGarden {
    ...
    vector<string> rearrange(vector<string> garden) {
		...
		int ct[26]={0};
		rep(i,2)rep(j,n) ++ct[garden[i][j]-'a'];

		rep(i,26)
		{
			if(ct[i]>n) return vector<string>();
		}

		vector<p> can;
		rep(i,26)
		{
			if(ct[i]>0) can.pb(p(ct[i],i+'a'));
		}
		sort(all(can),greater<p>());

		vector<string> ret(2,string(n,' '));
		int now=0;
		rep(i,n)
		{
			ret[i%2][i]=can[now].se;
			--can[now].fi;
			if(can[now].fi==0) ++now;
		}
		rep(i,n)
		{
			ret[(i+1)%2][i]=can[now].se;
			--can[now].fi;
			if(can[now].fi==0) ++now;
		}

		//printf("\n");
		//rep(i,2) std::cout << ret[i] << std::endl;
